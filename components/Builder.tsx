...
const COLORS = {
  ...
  textMain: "#f0f6fc",
  textMuted: "#8b949e",
  accent: "#B500B5" // Glavna boja (Magenta/Ljubičasta)
};
...
function BuilderContent() {
  ...
  const checkBottleneck = () => {
    ...
    if (gpuScore >= 3 && cpuScore <= 2 && (gpuScore - cpuScore >= 2)) return "⚠️ Upozorenje: Procesor je znatno slabiji od grafičke kartice.";
    return null;
  };
  ...
  useEffect(() => { 
    if (activeProduct && activeProduct.variants?.edges?.[0]?.node?.id) {
  ...
  const currentTotal = () => {
    const sum = [cpu, mb, ram, gpu, gpu2, ssd, ssd2, hdd, hdd2, pcCase, psu, cooler, os].reduce((acc, p) => acc + Number(p?.selectedVariant?.price?.amount || p?.variants?.edges?.[0]?.node?.price?.amount || 0), 0);
    return isReviewStep ? sum + ASSEMBLY_FEE : sum;
  };
  ...
  if (loading) return <div style={{ padding: "100px", textAlign: "center", color: COLORS.textMain }}>Učitavanje kompjuterskih komponenti...</div>;
          ...
          {!isReviewStep && (
            <>
              ...
              <div style={{ display: "flex", gap: "10px", marginBottom: "30px" }}>
                {stepIndex > 0 && <button onClick={() => setStepIndex(stepIndex - 1)} style={navBtnStyle}>← NAZAD</button>}
                {stepIndex > 0 && <button onClick={() => { setStepIndex(0); setBrand(null); }} style={{ ...navBtnStyle, color: "#ff4d4d" }}>🔄 ISPOČETKA</button>}
                {["hdd", "os"].includes(currentStep) && <button onClick={() => setStepIndex(stepIndex + 1)} style={{...navBtnStyle, marginLeft: "auto"}}>PRESKOČI ⏭</button>}
              </div>
            </>
          )}
          ...
          // REVIEW STEP (The new design)
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "25px", paddingRight: isMobile ? "0" : "20px" }}>
              <div style={{ display: "flex", gap: "10px", marginBottom: "10px" }}>
                ...
                <button onClick={() => { setStepIndex(0); setBrand(null); }} style={{ ...navBtnStyle, color: "#ff4d4d" }}>🔄 KRENI ISPOČETKA</button>
              </div>
                ...
                <div key={part.key} style={{ background: COLORS.bgCard, border: `1px solid ${COLORS.border}`, borderRadius: "12px", padding: isMobile ? "20px 15px" : "20px 30px", position: "relative", display: "flex", alignItems: "center", gap: "20px" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: "12px", color: COLORS.textMuted, marginTop: "4px" }}>
                      {part.item?.pcfSocket?.value || part.item?.pcfFormFactor?.value || part.item?.pcfWattage?.value || "Standardna Komponenta"}
                    </div>
                  </div>
                  <div style={{ fontSize: "18px", fontWeight: "bold", color: COLORS.textMain }}>
                    €{part.item?.selectedVariant?.price?.amount || part.item?.variants?.edges?.[0]?.node?.price?.amount || "0.00"}
                  </div>
                </div>
              ))}
              ...
              <div style={{ marginTop: "20px" }}>
                <h3 style={{ color: COLORS.textMain, fontSize: "18px", borderBottom: `1px solid ${COLORS.border}`, paddingBottom: "10px", marginBottom: "15px" }}>Dodatne Komponente</h3>
                <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                  <UpsellRow label="2. Grafička kartica" item={gpu2} onAdd={() => setAddingExtra("gpu2")} onRemove={() => setGpu2(null)} isAdding={addingExtra === "gpu2"} />
                  {addingExtra === "gpu2" && !gpu2 && <UpsellDropdown type="gpu" onSelect={(p:any) => {setGpu2(p); setAddingExtra(null);}} products={products} />}
                  
                  <UpsellRow label="Dodatni SSD" item={ssd2} onAdd={() => setAddingExtra("ssd2")} onRemove={() => setSsd2(null)} isAdding={addingExtra === "ssd2"} />
                  {addingExtra === "ssd2" && !ssd2 && <UpsellDropdown type="ssd" onSelect={(p:any) => {setSsd2(p); setAddingExtra(null);}} products={products} />}
                  
                  <UpsellRow label="Dodatni HDD" item={hdd2} onAdd={() => setAddingExtra("hdd2")} onRemove={() => setHdd2(null)} isAdding={addingExtra === "hdd2"} />
                  ...
                  return (
                    <div key={p.id} onClick={() => { if(isActive) handleSelection(p); else setActiveIndex(idx); }}
                      ...
                      <div style={{ fontSize: "20px", fontWeight: "bold", color: isActive ? COLORS.accent : COLORS.textMain, marginTop: "auto" }}>
                        €{p.variants?.edges?.[0]?.node?.price?.amount || "0.00"}
                      </div>
                      
                      {isActive && <div style={{ fontSize: "10px", color: COLORS.textMuted, marginTop: "10px", letterSpacing: "1px" }}>KLIKNI ZA ODABIR</div>}
                    </div>
                  );
                })}
              </div>
          ...
          <div style={{ background: COLORS.bgCard, border: `1px solid ${COLORS.border}`, borderRadius: "16px", padding: "30px", position: isMobile ? "relative" : "sticky", top: "40px" }}>
            ...
            <div style={{ display: "flex", gap: "10px", marginTop: "20px" }}>
              <div style={trustBadgeStyle}>
                ...
                <div>
                  <div style={{ fontSize: "11px", fontWeight: "bold", color: COLORS.textMain }}>Besplatna dostava</div>
                  <div style={{ fontSize: "9px", color: COLORS.textMuted }}>3-5 radnih dana</div>
                </div>
              </div>
              <div style={trustBadgeStyle}>
                <span style={{ fontSize: "16px", color: COLORS.accent }}>🛡️</span>
                <div>
                  <div style={{ fontSize: "11px", fontWeight: "bold", color: COLORS.textMain }}>2-godine jamstva</div>
                  <div style={{ fontSize: "9px", color: COLORS.textMuted }}>Pokriće svih kvarova</div>
                </div>
              </div>
            </div>
...
function UpsellRow({ label, item, onAdd, onRemove, isAdding }: { label: string, item: ProductNode | null, onAdd: () => void, onRemove: () => void, isAdding: boolean }) {
  ...
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "15px", borderRadius: "12px", background: COLORS.bgDark, border: `1px solid ${COLORS.border}`, fontSize: "14px" }}>
      ...
      <button onClick={onRemove} style={{ color: "#ff4d4d", border: "none", background: "none", cursor: "pointer", fontWeight: "bold" }}>✖ Ukloni</button>
    </div>
  );
}

function UpsellDropdown({ type, onSelect, products }: { type: string, onSelect: (p: ProductNode) => void, products: ProductNode[] }) {
...
const checkoutBtnStyle: CSSProperties = { width: "100%", padding: "20px", fontWeight: "bold", cursor: "pointer", borderRadius: "12px", fontSize: "18px", border: "none" };
