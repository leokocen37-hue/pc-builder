...
const STEP_LABELS: Record<Step, string> = {
  ...
  review: "Konfiguracija komponenti"
};

const ASSEMBLY_FEE = 200;

// --- COLOR PALETTE (Dark Theme + Custom Accent) ---
const COLORS = {
  bgMain: "#0a0c10",
  bgCard: "#161b22",
  bgDark: "#010409",
  ...
  textMain: "#f0f6fc",
  textMuted: "#8b949e",
  accent: "#B500B5" // Nova glavna boja
};
...
function BuilderContent() {
  ...
  const checkBottleneck = () => {
    ...
    if (gpuScore >= 3 && cpuScore <= 2 && (gpuScore - cpuScore >= 2)) return "⚠️ Upozorenje: Vaš procesor je slabiji od odabrane grafičke kartice.";
    return null;
  };
    ...
    <div style={{ background: COLORS.bgMain, minHeight: '100vh', width: '100%', color: COLORS.textMain, fontFamily: "system-ui, -apple-system, sans-serif", overflowX: "hidden" }}>
      <div style={{ maxWidth: "1400px", margin: "0 auto", padding: isMobile ? "20px 10px" : "40px 20px", display: "flex", flexDirection: isMobile ? "column" : "row", gap: "30px" }}>
        ...
        <div style={{ flex: 3, display: "flex", flexDirection: "column", minHeight: isMobile ? "auto" : "85vh" }}>
          ...
          {/* Navigation Buttons (Always visible after Step 0) */}
          {stepIndex > 0 && (
            <div style={{ display: "flex", gap: "10px", marginBottom: "30px" }}>
              <button onClick={() => setStepIndex(stepIndex - 1)} style={navBtnStyle}>← NAZAD</button>
              <button onClick={() => { setStepIndex(0); setBrand(null); }} style={{ ...navBtnStyle, color: "#ff4d4d" }}>🔄 ISPOČETKA</button>
              {!isReviewStep && ["hdd", "os"].includes(currentStep) && (
                <button onClick={() => setStepIndex(stepIndex + 1)} style={{...navBtnStyle, marginLeft: "auto"}}>PRESKOČI ⏭</button>
              )}
            </div>
          )}
          ...
          <div style={{ background: COLORS.bgCard, border: `1px solid ${COLORS.border}`, borderRadius: "16px", padding: "30px", position: isMobile ? "relative" : "sticky", top: "40px" }}>
            ...
            <div style={{ marginBottom: "25px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: "12px", color: COLORS.textMuted, marginBottom: "8px" }}>
                <span>Potrošnja:</span> <span>{estimatedDraw}W / {psuCapacity || "---"}W</span>
              </div>
            ...
            <div style={{ display: "flex", gap: "10px", marginTop: "20px" }}>
              ...
              <div style={trustBadgeStyle}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: "9px", color: COLORS.textMuted }}>Potpuna pokrivenost</div>
                </div>
              </div>
            </div>
